...
def calculate_demographic_data(print_data=True):
  ...
  percentage_bachelors = df.loc[
      df['education'] == 'Bachelors', 'education'].count() / df.shape[
          0]  #find the Bachelors and return the number rows for bachelor's in education column and divide by total of all rows
  ...
  higher_education_rich = higher_education.loc[
      ...
      'salary'].count() / higher_education.shape[0]
  ...
  lower_education_rich = lower_education.loc[
      ...
      'salary'].count() / lower_education.shape[0]
  ...
  rich_percentage = num_min_workers.loc[
      num_min_workers['salary'] == '>50K',
      'salary'].count() / num_min_workers.shape[0]
  rich_percentage = round(rich_percentage * 100, 1)

  # What country has the highest percentage of people that earn >50K?
  highest_earning = df.loc[
      df['salary'] == '>50K',
      'native-country'].value_counts() / df['native-country'].value_counts(
      )  #Get the number of people in each country that earn >50K and the percentage for each country and store Series format
